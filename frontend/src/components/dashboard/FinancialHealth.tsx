      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <div className="w-full flex items-center justify-between px-4">
          <div className="flex flex-col items-center">
            <span className="text-[9px] font-bold text-gray-600 uppercase tracking-widest">Savings Rate</span>
            <span className="text-[11px] font-semibold text-emerald-400 mt-1 tabular-nums">
              {Math.max(0, Math.round(summary.savingsRate))}%
            </span>
          </div>
          <div className="h-6 w-px bg-white/[0.05]" />
          <div className="flex flex-col items-center">
            <span className="text-[9px] font-bold text-gray-600 uppercase tracking-widest">Expense Ratio</span>
            <span className="text-[11px] font-semibold text-white mt-1 tabular-nums">
              {summary.income.current > 0
                ? `${Math.min(100, Math.round((summary.expenses.current / summary.income.current) * 100))}%`
                : "100%"}
            </span>
          </div>
        </div>
